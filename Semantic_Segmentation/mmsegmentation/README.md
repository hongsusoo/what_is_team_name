...
# MMSegmentation Revision for 재활용 품목 분류를 위한 Semantic Segmentation
## 수정 사항
- Albumentations 적용 기능 추가
  - ```dict(type='Albumentations', transform='MotionBlur', blur_limit=7, p=0.1)``` 형식으로 data pipeline config에 추가하여 사용 가능

- tools/submission.py
  - AI Stage 제출용 csv 파일 생성
  - pseudo labeling 데이터 용 csv 파일 생성
  - 다른 모델과의 soft-voting ensemble을 위한 logit 저장 기능 (EncoderDecoder 모듈이 수정되었음)
  - 사용 방법
```bash
python ./tools/submission.py <config file path> <model checkpoint file path> <output csv file path> --output_logit_pkl_path <output logit pkl path>
```
--output_logit_pkl_path 미기재시 logit 저장하지 않음<br/>
--for_pseudo flag 있을 경우, csv file에는  512x 512에 대한 출력 저장 (pseudo 라벨링 용)<br/>
예시
```bash
python ./tools/submission.py \
./boostcamp/recycle_mb_gd_upernet_swin_base_patch4_window12_512x512_160k_ade20k_pretrain_384x384_22K.py \
./work_dir/upernet_swin_albu_mb_gd_40k/latest.pth \
./work_dir/output_test/submission_upernet_swin_albu_mb_gd_40k_512.csv \
--for_pseudo \
--out_logit_pkl_path ./work_dir/output_test/upernet_swin_albu_mb_gd_40k_logit.pkl
```
## License
MMSegmentation 의 라이센스 정책을 그대로 따릅니다. <br/> 
즉, this project is released under the [Apache 2.0 license](LICENSE).
